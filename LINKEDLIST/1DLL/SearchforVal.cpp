...
bool valPresentorNot(Node* head,int val){
    //? means while traversing the list we have to check that at any node has data == val or not  
    Node* temp = head;
    while(temp!= nullptr){
        if(temp->data == val){
            return true;
