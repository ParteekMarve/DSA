...
//     for (int i = 0; i < n; i++) {
//         int more_require = tarnum - arr[i];  //!its like a = b + c where b is known and a - c gives b
//         if (mpp.count(more_require)) {   //!   mpp.count(temp) returns the number of times temp appears as a key in the map
                                        /*For unordered_map, it returns either:
                                            1 if the key exists
                                            0 if the key doesn't exist */
               //! Alternates ways to check presence of temp {key}
              /*mpp.count( ) != 0
                mpp.find(more_required) != mpp.end()
                mpp.contains(more_required) (C++20 and later)*/
//             cout << more_required << " " << arr[i] << endl;
//             return true;
//         }
...
int main(){
  ...
  // Create a vector of pairs to remember original indices after sorting
    ...
    sort(arr,arr+n);
    while(left< right){
        int sum = numsWithIndex[left].second + numsWithIndex[right].second;
        if(sum == tarnum){
            cout<< numsWithIndex[left].second << numsWithIndex[right];
